#ifndef TIME_EXPANDED_MULTI_ALT_H
#define TIME_EXPANDED_MULTI_ALT_H
...
class TspMultiALT
{
 ...
 public:
    ...
    /**
     * @brief Constructor. Creates an instance of TspMultiALT.
     ...
     */
    TspMultiALT( GraphType& graph, StationContainer& stationContainer, VehicleContainer& vehicleContainer, TTL* timestamp): 
    G(graph), stations(stationContainer), vehicles(vehicleContainer), m_timestamp(timestamp)
    {
        pq.reserve( G.getNumNodes());
        selectedVehicles.resize( vehicles.size(), false);
    }
     ...
     **/
    void runQuery( const StationID depStationId, const StationID arrStationId, const Time startTime, std::vector<VehicleTypeID>& vehicleTypes)
    {
        NodeIterator u;

        init( depStationId, arrStationId, startTime, vehicleTypes);
        ...
        while( !pq.empty())
        {
            ...
            {
                /*if( u->dist > ( stations[u->stId].minDist + stations[u->stId].transitTime))
                {
                   stations[u->stId].timestamp = (*m_timestamp);
                   stations[u->stId].isSettled = true;
                }*/
 ...
 private:
     ...
     * @param startTime The start time at the departure-station.
     **/
    void init( const StationID depStationId, const StationID arrStationId, const Time startTime, std::vector<VehicleTypeID>& vehicleTypes)
    {
        pq.clear();
        ++(*m_timestamp);
        target = G.endNodes();
    ...
    /**
     * @brief Disables all vehicle types.
     */
