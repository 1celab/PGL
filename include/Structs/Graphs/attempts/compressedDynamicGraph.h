 ...
 *
 * @tparam GraphImplementation The underlying graph implementation to use. Available implementations are 'PackedMemoryArrayImpl' and 'AdjacencyListImpl'
...
public:
    ...
    {
        return EdgeDescriptor( getNodeDescriptor( source(e)), getNodeDescriptor(target(e)));
    }
    ...
    {
        return EdgeDescriptor( getNodeDescriptor( source(k)), getNodeDescriptor(target(k)));
    }
    ...
    bool hasValidInEdges()
    {
        ...
        for( u = beginNodes(), end_nodes = endNodes(); u != end_nodes; ++u)
        {
            ...
            {
                if( getEdgeDescriptor(e) != getEdgeDescriptor(getInEdgeIterator(e)))
                {
                    return false;
                }
