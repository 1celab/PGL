...
public:
     ...
     */
    AStarDijkstra( GraphType& graph, unsigned int* timestamp):G(graph),m_timestamp(timestamp)//,m_maxSpeed(0) // CHANGE
    {
        // CHANGE
        /*
    ...
    bool hasFeasiblePotentials( const typename GraphType::NodeIterator& t)
    {
        ...
        for( u = G.beginNodes(), lastNode = G.endNodes(); u != lastNode; ++u)
        {
            ...
            {
                v = G.target(e);
                reducedCost = e->weight - u->heuristicValue + v->heuristicValue;
                if( reducedCost < 0)
                {
                    std::cout << "(" << G.getRelativePosition(u) << "," << G.getRelativePosition(v) << ") -> " << G.getRelativePosition(t) << "\n";
                    std::cout << "p(u)  = " << u->heuristicValue << "\n";
                    std::cout << "p(v)  = " << v->heuristicValue << "\n";
    ...
    WeightType getHeuristic( const NodeIterator& u, const NodeIterator& t)
    {
//        return euclideanDistance( u->x, u->y, t->x, t->y)/m_maxSpeed;
        return u->heuristicValue;
    }
...
//        if (not hasFeasiblePotentials(t))
//          std::cout << "Inside comb() (PGL's A*): hasFeasiblePotentials(t) "
//                    << "reported a problem, please check it!" << std::endl;
        ...
        while( !pq.empty())
        {
            ...
            {
                v = G.target(e);
                reducedCost = e->weight + v->heuristicValue - u->heuristicValue; 
