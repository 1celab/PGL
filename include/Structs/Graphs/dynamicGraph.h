 ...
 *
 * @tparam GraphImplementation The underlying graph implementation to use.
 * Available implementations are 'PackedMemoryArrayImpl' and 'AdjacencyListImpl'
 * @tparam Vtype The data to associate with the nodes. It must be a class or struct
 * @tparam Etype The data to associate with the outgoing edges. It must be a class or struct
 * @tparam InEtype The data to associate with the incoming edges. It must be a class or struct
 * @author Panos Michail
 * @version 1.1 (25-2-2014, latest minor updates were done by Andreas Paraskevopoulos)
 */

template<template <typename vtype,typename etype,typename inetype> class GraphImplementation, typename Vtype = DefaultGraphItem, typename Etype = DefaultGraphItem, typename InEtype = Etype>
class DynamicGraph
{
public:
    typedef typename GraphImplementation<Vtype,Etype,InEtype>::SizeType         SizeType;
    typedef typename GraphImplementation<Vtype,Etype,InEtype>::NodeDescriptor   NodeDescriptor;
    typedef std::pair<NodeDescriptor,NodeDescriptor>                            EdgeDescriptor;
    typedef typename GraphImplementation<Vtype,Etype,InEtype>::NodeIterator     NodeIterator;
    typedef typename GraphImplementation<Vtype,Etype,InEtype>::EdgeIterator     EdgeIterator;
    typedef typename GraphImplementation<Vtype,Etype,InEtype>::InEdgeIterator   InEdgeIterator;
    typedef unsigned int                                                        PropertyType;
    typedef Vtype                                                               NodeData;
    typedef Etype                                                               EdgeData;
    typedef InEtype                                                             InEdgeData;

    DynamicGraph():m_numNodes(0),m_numEdges(0)
    {
        impl = new GraphImplementation<Vtype,Etype,InEtype>();
    }
     ...
     *
     * @param u The node
     * @param e The iterator to the first outgoing edge of the node
     */
    void beginEdges( const NodeIterator& u, EdgeIterator& e) const
    {
        e = impl->beginEdges( u);
    }
     ...
     *
     * @param u The node
     * @param e The iterator to the first incoming edge of the node
     */
    void beginEdges( const NodeIterator& u, InEdgeIterator& k) const
    {
        k = impl->beginInEdges( u);
    }
    ...
    GraphImplementation<Vtype,Etype, InEtype>& getImpl()
    {
       return *impl;
    }
     ...
     */
    bool hasEdge( const EdgeDescriptor& descriptor) const
    {
    ...
    /**
     * @brief Returns the end to the container of the outgoing edges of a node
     *
     * @param u The node
     * @param e The iterator to the past-the-end outgoing edge of the node
     */
    void endEdges( const NodeIterator& u, EdgeIterator& e) const
    {
        e = impl->endEdges( u);
    }
     ...
     *
     * @param u The node
     * @param e The iterator to the past-the-end incoming edge of the node
     */
    void endInEdges( const NodeIterator& u, InEdgeIterator& k) const
    {
        k = impl->endInEdges( u);
    }
     ...
     * @brief Erases an edge from the graph
     *
     * @param e The iterator of the edge to be erased
     */
    void eraseEdge( EdgeIterator e)
    {
     ...
     * @brief Erases a node from the graph
     *
     * @param v The iterator of the node to be erased
     */
    void eraseNode( NodeIterator v)
    {
        eraseNode( getNodeDescriptor( v));
    }
     ...
     */
    EdgeDescriptor getEdgeDescriptor( const NodeDescriptor& uD, const NodeDescriptor& vD) const
    {
     ...
     * @return The descriptor of the edge between u and v
     */
    EdgeDescriptor getEdgeDescriptor( const NodeIterator& u, const NodeIterator& v) const
    {
        assert( hasEdge(u,v));
     ...
     */
    EdgeDescriptor getEdgeDescriptor( const EdgeIterator& e) const
    {
     ...
     *
     */
    EdgeDescriptor getEdgeDescriptor( const InEdgeIterator& k) const
    {
     ...
     */
    EdgeIterator getEdgeIterator( const EdgeDescriptor& descriptor) const
    {
    ...
    {
        return impl->getEdgeIterator( u, v);
    }

    /**
     * @brief Returns an inedge iterator to an outgoing edge
     *
     * @param descriptor The descriptor of the edge
     * @return An iterator to the outgoing edge
     */
    ...
    {
        return impl->getInEdgeIterator( uD, vD);
    }
    ...
    {
        return impl->getInEdgeIterator( u, v);
    }
     ...
     */
    NodeDescriptor getNodeDescriptor( const NodeIterator& u) const
    {
     ...
     * @return An iterator to the node
     */
    NodeIterator getNodeIterator( const void* descriptor) const
    {
     ...
     * @brief Inserts an edge in the graph
     *
     * @param u The iterator of the source node of the edge
     * @param v The iterator of the target node of the edge
     *
     * @return The descriptor of the new edge, zero if the operation failed
     */
    EdgeDescriptor insertEdge( NodeIterator u, NodeIterator v)
    {
        return insertEdge( getNodeDescriptor( u), getNodeDescriptor( v));
    }

    /**
     * @brief Changes the head node of the edge.
     *
     * @param e The edge
     * @param v The new head node of the edge.
     */
   ...
   /**
     * @brief Swaps the outgoing edges of a node.
     *
     * @param e1 The fist edge.
     * @param e2 The second edge.
     */
    void swapEdges( EdgeIterator& e1, EdgeIterator& e2)
    {
        //impl->swapEdges( e1, e2); return;
        ...
        /*InEtype* inedge1 = &(*k1);
        InEtype* inedge2 = &(*k2);
        InEtype inedgetemp = *k1;

        (*inedge1) = (*inedge2);
        (*inedge2) = inedgetemp;*/

        NodeDescriptor u1D = getNodeDescriptor(source( k1));
        NodeDescriptor u2D = getNodeDescriptor(source( k2));
        assert( u1D == u2D);

        NodeDescriptor v1D = getNodeDescriptor(target( e1));
        NodeDescriptor v2D = getNodeDescriptor(target( e2));

        e1->m_adjacentNode = *v2D;
        e2->m_adjacentNode = *v1D;

        k1->m_adjacentNode = *u2D;
        k2->m_adjacentNode = *u1D;
   ...
   /**
    ...
    void swapEdges( InEdgeIterator& k1, InEdgeIterator& k2)
    {
        ...
        NodeDescriptor u1D = getNodeDescriptor(target( e1));
        NodeDescriptor u2D = getNodeDescriptor(target( e2));
        assert( u1D == u2D);

        NodeDescriptor v1D = getNodeDescriptor(source( k1));
        NodeDescriptor v2D = getNodeDescriptor(source( k2));

        k1->m_adjacentNode = *v2D;
        k2->m_adjacentNode = *v1D;

        e1->m_adjacentNode = *u2D;
        e2->m_adjacentNode = *u1D;
    ...
    {
        const InEdgeIterator k = getInEdgeIterator(e);
        return source( k);
    }
    ...
    /**
     * @brief Returns the head (target) node for an outgoing edge or the tail (source) node for an incoming edge
     *
     * @tparam EIterator The incoming or outgoing edge of a node
     * @return The target/source node of the outgoing/incoming edge
     */
    template<typename EIterator>
    NodeIterator opposite( const EIterator& e) const
    {
    ...
    void switchNodeDescriptors( const NodeDescriptor& first, const NodeDescriptor& second)
    {
    ...
    {
        const EdgeIterator e = getEdgeIterator(k);
        return target(e);
    }
...
private:
    GraphImplementation<Vtype,Etype, InEtype>*   impl;
    SizeType                                     m_numNodes;
    SizeType                                     m_numEdges;
};
