...
public:
	...
	if( newBucket == NULL)
	{
	   std::cout << "new has failed... !!!!\n";
	}
        bucket->m_capacity *= 2;
        registerBucket( newBucket, bucket->m_capacity);
    ...
    void swap( BucketSet& ext)
    {
        ext.m_pool.swap(m_pool);
    }
...
public:
    ...
    {
        m_bucketSet = bucket.m_bucketSet;
        m_begin = bucket.m_begin;
        m_capacity = bucket.m_capacity;
        m_numElements = bucket.m_numElements;
    }
    ...
    Iterator atIndex( const SizeType& pos)
    {
        return m_bucketSet->atIndex(m_begin + pos);
    }
