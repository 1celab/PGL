#ifndef DYNTM_MULTI_ALT
#define DYNTM_MULTI_ALT
...
class TspMultiALT
{
            ...
            for( unsigned int i=0, ssize=currSt.adjStations.size(); i<ssize; ++i)
            {
                ...
                for( unsigned int j=0; j<usedVehicles.size(); j++)
                {
                    ...
                    for( e = currSt.getEdge( u->time, i, j), endEdge = currSt.stVehEdges[i][j+1]; e != endEdge; ++e)
                    {
                        ...
                        if( selectedVehicles[depNode->tpId] == false) continue;
                    ...
                    //for( unsigned int j=0, endj=nextDay.size(); j < endj; j++)
                    {
                        //e = nextDay[j];
                        depNode = G.target( e);
                        assert( depNode->isDeparture());
 ...
 private:
    ...
    std::vector<EdgeIterator> nextDay;
