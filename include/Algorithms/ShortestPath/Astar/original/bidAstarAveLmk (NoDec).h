...
class BidAstarAveLmk
{
 ...
 public:
    ...
    typedef PriorityQueueNoDec< WeightType, NodeIterator, HeapStorage>   PriorityQueueType;
 ...
 private:
    ...
    void searchForward()
    {
        ...
        u = pqFront.minItem();
        discoveredDistance = pqFront.minKey();
        pqFront.popMin();
        ++m_numSettledNodes;

        //if( u->dist != discoveredDistance) return;
        ...
        for( e = G.beginEdges(u), lastEdge = G.endEdges(u); e != lastEdge; ++e)
        {
            ...
            if( v->timestamp < (*m_timestamp))
            {
                ...
                pqFront.insert( forwardLBdist(discoveredDistance, v), v);
            }
    ...
    void searchBackward()
    {
        ...
        u = pqBack.minItem();
        discoveredDistance = pqBack.minKey();
        pqBack.popMin();
        ++m_numSettledNodes;

        //if( u->distBack != discoveredDistance) return;
        ...
        for( k = G.beginInEdges(u), lastInEdge = G.endInEdges(u); k != lastInEdge; ++k)
        {
            ...
            if( v->timestamp < (*m_timestamp))
            {
                ...
                pqBack.insert( backwardLBdist(discoveredDistance, v), v);
            }
