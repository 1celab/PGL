...
void update( TspNetwork& net)
{
    ...
    if( net.stations[stationId].numDepEvents == 0)
    {
        std::cout << "\nThere are no depNodes in the station!";
        return;
    }

    //get the ealiest departure after the start time
    typename TspNetwork::NodeIterator firstDepNode = net.G.getNodeIterator( net.stations[stationId].firstDepNode);
    typename TspNetwork::NodeIterator depNode = firstDepNode;
    for( unsigned int i = 0; i < net.stations[stationId].numDepEvents; i++)
    {
        if( startTime <= depNode->time)
            break;

        depNode = net.G.target(++net.G.beginEdges( depNode));
    }
    ...
    for( StationID i=0, size=net.stations.size(); i<size; i++)
        net.isValid( i);
...
std::string queryFileName, std::string resultFileName, std::string& basePath, bool skipDelayUpdates = false)
{
    ...
    for( unsigned int i=0; i < numQueries; ++i)
    {
        ...
        else if( skipDelayUpdates == false)
        {
            ...
            //skip the query, only if station has no departure events
            const typename TspNetwork::Station & station = net.stations[queries[i].stationId];
            if( station.numDepEvents != 0)
            {
                ...
                //    if( net.checkValidity( j) == false)
                //        std::cout << "d " << queries[i].stationId << " " << queries[i].delay << " " << queries[i].startTime;
...
void shell( int& format, int& mod, int& algorithm, std::string& basePath, 
            std::string& qFile, std::string& rFile, std::string& netName)
{
    typedef TspNetwork<GraphType> TspNetwork;
    ...
    else
    {
        ...
        {
            algorithm = 1;
            rFile = "./stats/" + netName + ".RTE.RDJ.results";
            runQueriesFromFile( net, algorithm, qFile, rFile, basePath, true);

            algorithm = 2;
            rFile = "./stats/" + netName + ".RTE.ODJ.results";
            runQueriesFromFile( net, algorithm, qFile, rFile, basePath, true);
    
            algorithm = 3;
            rFile = "./stats/" + netName + ".RTE.ALT.results";
            runQueriesFromFile( net, algorithm, qFile, rFile, basePath, true);

            algorithm = 4;
            rFile = "./stats/" + netName + ".RTE.MDJ.results";
            runQueriesFromFile( net, algorithm, qFile, rFile, basePath, true);

            algorithm = 5;
            rFile = "./stats/" + netName + ".RTE.MALT.results";
...
int main( int argc, char* argv[])
{
    ...
    switch( graphVariant)
    {
        //ADJ
        case 1:
            shell<Graph>( format, mod, algorithm, basePath, qFile, rFile, netName);
            break;

        //dynFS
        case 2:
            shell<fsGraph>( format, mod, algorithm, basePath, qFile, rFile, netName);
            break;

        //PMG
        case 3:
            shell<pmaGraph>( format, mod, algorithm, basePath, qFile, rFile, netName);
            break;
