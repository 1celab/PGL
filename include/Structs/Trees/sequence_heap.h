...
public:
  ...
  const Value& getMinValueRef() const { return data[1].value; }
  const Key& getMinKeyRef() const { return data[1].key; }
  void  deleteMin();
...
sortTo(Element *to)
{
  ...
  while (to < beyond) {
    ...
    *to = *root;
    ...
    //andreas added to avoid a reset step
    data[sz + 1].key = sup;
...
template <class Key, class Value>
inline void KNHeap<Key, Value>::clear()
{
  while( !empty())
      popMin();
...
template <class Key, class Value>
inline void KNHeap<Key, Value>::reserve(std::size_t)
{
   ;
}
...
template <class Key, class Value>
bool KNHeap<Key, Value>::empty() const
{
  return (getSize() == 0);
}
...
template <class Key, class Value>
inline const Value&  KNHeap<Key, Value>::minItem()  const
{
  if (insertHeap.getMinKeyRef() >= minBuffer1->key) {
    return minBuffer1->value;
  } else {
    return insertHeap.getMinValueRef();
  }
}

template <class Key, class Value>
inline const Key&  KNHeap<Key, Value>::minKey()  const
{
  if (insertHeap.getMinKeyRef() >= minBuffer1->key) {
    return minBuffer1->key;
  } else {
    return insertHeap.getMinKeyRef();
  }
}
...
template <class Key, class Value>
inline void KNHeap<Key, Value>::popMin() {
  Key key1 = minBuffer1->key;
  Key key2 = insertHeap.getMinKey();
  if (key2 >= key1) {
    Assert2(minBuffer1 < buffer1 + KNBufferSize1); // no delete from empty
